...
class Agent(object):
    ...
    def __init__(self, csvParameters, ksi = 0.5, dhor = 10, timehor = 5, goalRadiusSq = 1, maxF = 10):
        ...
        self.vel_uncertain = 0.0 # The velocity uncertainty constant
        self.m = 1 # Some constant used in the PowerLaw Model
        self.tau_0 = 3 # Exponential cutoff point
        self.k = 1.272 # PowerLaw scaling model
    ...
    # Function to calculate teh discriminant
    def D(self, j):
        rad = self.radius + j.radius
        w = self.pos - j.pos
        c = np.dot(w, w) - (rad*rad)
    ...
    def computeForces(self, neighbors=[]):
        ...
        ttc_fin = np.inf
        for i in neighbors:
            if self.id != i.id and not i.atGoal: # Computing avoidance forces for neighbors in the PowerLaw Model
                ...
                if obj_sense_dist < 10:
                    ...
                    if ttc_val != np.inf:
                        if ttc_val < ttc_fin: 
                            ...
                            rad1 = self.radius + i.radius
                            Favoid_dir = rel_pos + (ttc_fin*rel_vel)
                            term1 = (self.k * math.exp(-ttc_fin/self.tau_0)) / (ttc_fin**(self.m + 1) + 1e-10)
                            term2 = self.m + (ttc_fin / self.tau_0)
                            term3 = 1 / np.sqrt(self.D(i))
                            Favoid_mag = term1 * term2 * term3
